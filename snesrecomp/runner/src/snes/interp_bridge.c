...
#define SNESRECOMP_LLE_BOUNCE_DEFAULT 0
#endif
...
int interp_bridge_scheduler_aot_enabled(void) {
    return lle_yield_bounce_enabled();
}
...
static int _interp_run_core(CpuState *cpu, uint32_t entry_pc24,
    ...
    for (; steps < step_cap; steps++) {
        ...
        /* A caller-supplied master deadline already guarantees a bounded
         * yield.  The repeated-state search is only needed when a run may
         * otherwise spin indefinitely; short SMK decompressor slices enter
         * this loop hundreds of thousands of times per transition. */
        if (auto_quiescent && !s_lle_master_deadline) {
            /* Most deadline-bounded calls execute only one or two opcodes.
             * Clear the repeat detector only if a call actually needs it;
             * the deadline may change during a nested run. */
            if (!qring_initialized) {
        ...
        if (_cyc <= 0) _cyc = 1;
        {
            ...
            /* Charge actual bus accesses and internal CPU cycles separately.
             * Do not compensate for AOT scheduling with a per-opcode constant. */
        ...
        if (is_call) {
            ...
            if (bounce_ok && has_body) {
                ...
                if (yield_pc || auto_quiescent || s_interp_bounce_owner_depth <= 0)
