...
uint8 *RomPtr(uint32_t addr) {
  ...
  if (g_snes && g_snes->cart && g_snes->cart->type == CART_SUPERFX &&
      g_snes->cart->ram && g_snes->cart->ramSize > 0) {
    if ((bank < 0x40 || (bank >= 0x80 && bank < 0xc0)) &&
        lo >= 0x6000 && lo < 0x8000) {
      return &g_snes->cart->ram[(lo & 0x1fff) & (g_snes->cart->ramSize - 1)];
    }
    if (bank == 0x70 || bank == 0x71 || bank == 0x78 || bank == 0x79 ||
        bank == 0xf0 || bank == 0xf1) {
      return &g_snes->cart->ram[(((uint32_t)(bank & 1) << 16) | lo) &
                                (g_snes->cart->ramSize - 1)];
    }
  }
